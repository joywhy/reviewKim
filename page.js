...
const PAGE = {
  ''({
    ...
    */
    ...
    cond(html`<a href=#login class=mainBtn>로그인</a><div class="blind popup"><article id=login><button class="close ℓ"></button><img src=${IMG_DIR}/logo_en_reviewKim.svg><></article></div>`, $ => {
    const {firstChild: $start, lastChild: $end} = $;
    const $$ = ℓ($,
    comp_login
    );
    ...
    comp_timer,
    comp_map,
    cond(html`<section class=blind><form class=ℓ><a class="close ℓ"></a><span>체험단 신청하기</span><h1>[경기 / 하남] 마이 논알콜 하이볼 리스트</h1><div class=vote><em>복수선택</em><></div><label>리뷰 작성할 블로그 주소<input name=url required=true></label><div class=policy><span>유의사항 및 개인정보 활용</span><label><input type=checkbox required=true> 체험단 미션 수행 및 유의사항, 개인정보 제3자 제공에 동의합니다.</label></div><button type=submit class=mainBtn>신청</button></form></section>`, $ => {
    const {firstChild: $start, lastChild: $end} = $;
    const $$ = ℓ($,
    loop(html`<label><input type=checkbox name=date class=ℓ><div><em class=ℓ></em><span class=ℓ></span></div></label>`, $ => {
    const $$ = ℓ($,
    );
  ...
  my({my}){
    ...
    const $ = html`<header><div class=container><h1><a href=/ class=logo><img src=${IMG_DIR}/logo_kor_reviewKim.svg alt=리뷰킴><img src=${IMG_DIR}/logo_en_reviewKim.svg alt=reviewkim></a></h1><div class="rightSide center"><a href=//pf.kakao.com/_gQbVG/chat>비즈니스 문의</a><><></div></div></header><main class=userInfo><img class="userPhoto ℓ"><strong class=ℓ></strong><dl style="--dt: 80;"><dt>Name</dt><dd class=ℓ></dd><dt>Gender</dt><dd class=ℓ></dd><dt>Mail</dt><dd class=ℓ></dd><dt>Birthday</dt><dd class=ℓ></dd><dt>Tel</dt><dd class=ℓ></dd></dl><a href=?logout>로그아웃</a></main><footer><div class=container><div class=topFooter><h4>070-7630-1111</h4>평일 10:00 - 17:00 / 점심 시간 12:00 - 13:00<br>주말, 공휴일 제외</div><div class=bottomFooter><div><h6>개인정보처리방침 | 이용약관</h6>상호명: 리을컴퍼니 주소 : 경기 수원시 팔달구 고매로20번길 24 사업자등록번호: 183-42-00547<br>대표 : 박재천 이메일 :admin@lieul.com </div><a href=/ class=logo><img src=${IMG_DIR}/logo_kor_reviewKim.svg alt="리뷰 킴"><img src=${IMG_DIR}/logo_en_reviewKim.svg alt=reviewKim></a></div></div></footer>`
    const $$ = ℓ($,
    cond(html`<a href=/my><img src=${IMG_DIR}/user.svg><span class=ℓ></span></a><a href=?logout>로그아웃</a>`, $ => {
    const {firstChild: $start, lastChild: $end} = $;
    const $$ = ℓ($,
